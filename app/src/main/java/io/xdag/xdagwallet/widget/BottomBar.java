...
public class BottomBar extends LinearLayout {
    ...
    private List<BottomBarItem> mTabs = new ArrayList<>();
    ...
    public BottomBar addItem(final BottomBarItem tab) {
     ...
     * 获取 Tab
     */
    public BottomBarItem getItem(int index) {
    ...
    private void toggle(final boolean visible, final boolean animate, boolean force) {
        if (mVisible != visible || force) {
            ...
            if (height == 0 && !force) {
                ...
                if (vto.isAlive()) {
                    // view 树完成测量并且分配空间而绘制过程还没有开始的时候播放动画。
