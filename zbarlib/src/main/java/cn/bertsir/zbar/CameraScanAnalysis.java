    ...
    CameraScanAnalysis() {
        ...
        mHandler = new Handler(Looper.getMainLooper()) {
            ...
            public void handleMessage(Message msg) {
                if (mCallback != null) mCallback.onScanSuccess((String) msg.obj);
            }
        };
    }
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        if (allowAnalysis) {
            ...
            if (Symbol.is_only_scan_center) {
                int cropWidth = (int) (Symbol.cropWidth *
                    (size.height / (float) Symbol.screenWidth));
                int cropHeight = (int) (Symbol.cropHeight *
                    (size.width / (float) Symbol.screenHeight));
